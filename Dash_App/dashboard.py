...
app.layout = html.Div(children=[
    ...
    html.Div(['Input Year: ', dcc.Input(id='input-year', value=2010, type='number',
             style={'height': '35px', 'font-size': 30}),], style={'font-size': 30}),
    html.Br(),
    html.Br(),
    # segment 1
    html.Div([
...
def compute_info(data_frame: pd.DataFrame, entered_year):
    # Select data
    df = data_frame[data_frame['Year'] == int(entered_year)]
...
def get_graph(entered_year):
    ...
    late_fig = px.line(avg_late, x='Month', y='LateAircraftDelay', color='Reporting_Airline',
                       title='Average LateAircraft delay time (minutes) by airline')
